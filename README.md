...
- [x] **Этап 2:** Бэкенд-каркас и авторизация
  - Профиль, middleware защиты роутов, базовые UI-компоненты, сессии Supabase SSR
  - _Вход изначально был magic-link (Supabase email); заменён на вход по коду через Resend —
    см. раздел «Авторизация — вход по коду (Resend)» ниже._
- [x] **Этап 3:** Каталог
...
- [x] **Авторизация — вход по коду (Resend) + никнеймы** ✅
  - **Переход с magic-link Supabase на собственный 6-значный код**, который шлёт **Resend**
    (`https://api.resend.com/emails`, ключ `RESEND_API_KEY`, отправитель `noreply@nicetry.guru`).
    Сессия по-прежнему Supabase: после проверки кода сервер минтит обычную Supabase-сессию
    (`admin.generateLink` + `verifyOtp`, как dev-login), поэтому `users.id == auth.users.id` и весь
    остальной код (профиль/заказы/баланс/админка/middleware) работает без изменений.
  - **Поток входа (новая сессия):** ник ИЛИ почта → код на почту → ввод кода → вход. Если у аккаунта
    ещё нет ника — экран «придумайте ник» (live-проверка свободен/занят), затем автологин в профиль.
  - **Безопасность кодов:** в БД хранится только HMAC-SHA256-хеш кода (не сам код); TTL
    `AUTH_CODE_TTL_MINUTES` (10 мин); лимит попыток (5) + блок кода; rate-limit на отправку
    (1/60с и 6/час на почту, 30/час на IP). Не раскрываем существование аккаунта по нику.
    Без ключа Resend: dev — код в консоль; prod — внятная ошибка. Обработка `429/403/422/5xx` от Resend.
  - **Файлы:** `src/lib/resend.ts` (отправка), `src/lib/auth/codes.ts` (ген/хеш/проверка),
    `src/lib/auth/nickname.ts` (валидация), роуты `POST /api/auth/send-code`, `POST /api/auth/verify-code`,
    `POST /api/user/nickname`, `GET /api/user/nickname/check`, фронт `src/app/auth/login/page.tsx`.
    Старый magic-link роут `POST /api/auth/login` деактивирован (`410`), чтобы Supabase не слал письма.
  - **БД:** миграция `migrations/auth_resend.sql` — таблица `auth_codes` + колонка `users.nickname`
    (UNIQUE по `LOWER(nickname)`). Применить service-role'ом (см. «Нужно от владельца»).
  - **Статус:** код боевой, `npm run build` зелёный, протестирован вживую (вход по почте/нику, ник, автологин).
...
- [ ] **Гостевой чекаут + ник после оплаты** — отложено до подключения платёжного шлюза
  - По ТЗ: гость указывает почту → сразу оплачивает (СБП/карта) → экран «придумайте ник» → автологин.
  - **Завязано на Pay4game:** сейчас покупка = списание с внутреннего баланса, а у гостя без аккаунта
    баланса нет. Полноценный гостевой чекаут возможен только с внешним приёмом денег (не требует
    предзаведённого баланса). До этого чекаут — auth-first (нужен вход). Вход по коду (выше) уже готов.
...
5. **Платёжная система (Pay4game)** — card/crypto пока `501`; оплата с баланса работает.
   После подключения — доделать гостевой чекаут (почта → оплата → ник → автологин).
6. Проверить курс/наценку категории `dessly-games` в админке (влияет на цену гифтов).
7. **Авторизация (Resend):** вписать `RESEND_API_KEY` + `AUTH_SESSION_SECRET` в env (Vercel prod),
   верифицировать домен `nicetry.guru` в Resend (DNS: DKIM/SPF/DMARC), применить
   `migrations/auth_resend.sql` на боевой Supabase. Без этого вход по коду на проде не работает.
