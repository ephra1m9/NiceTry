...
import { DEFAULT_TELEGRAM_SETTINGS } from '@/lib/telegram-pricing'
...
/**
 * Управление настройками блока Telegram Stars / Telegram Premium из админки
 * (синглтон telegram_settings, id=1). Наценка% и курс USD→₽ — НЕ хардкод, редактируются здесь.
 * Всё под requireAdmin; запись через service-role (RLS пускает на запись только service-role).
 */

// GET /api/admin/telegram-settings — текущие настройки (с фолбэком на дефолты).
export async function GET() {
  try {
    ...
    const { data } = await guard.admin
      ...
      .select('markup_percent, usd_to_rub_rate, updated_at')
      .eq('id', 1)
      .maybeSingle()

    return NextResponse.json({ settings: data || { ...DEFAULT_TELEGRAM_SETTINGS } })
  } catch (error: any) {
...
// PATCH /api/admin/telegram-settings — обновление настроек (белый список + валидация).
